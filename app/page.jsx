...
export default function LandingPage() {
  return (
    <div className="flex flex-col pt-16 min-h-screen bg-linear-to-br from-blue-50 via-white to-blue-100">
      {/* ───── Hero ───── */}
          ...
          <div className="mx-auto mt-12 grid max-w-5xl gap-8 md:grid-cols-3">
            {STEPS.map(({ label, title, description }) => (
              <div key={label} className="flex flex-col items-center space-y-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-100 text-xl font-bold text-blue-900">
                  {label}
                </div>
                <h3 className="text-xl font-bold">{title}</h3>
      ...
      <footer className="border-t bg-gray-50 py-12 text-center text-sm text-muted-foreground">
        © {new Date().getFullYear()} Splitt. All rights reserved.
      </footer>
    </div>
  );
}